...
## Status: Phase 0 — pre-product UI

This repository currently contains **the deterministic core engine and the infrastructure around it**. The employee survey UI, personal-result screen, admin dashboard, Opportunity Map visualisation, exports UI and branding system are **not built yet**.

That ordering is intentional. A visually impressive application with incorrect scoring or privacy behaviour is a failure, so the methodology is proved with tests and synthetic data before any product screen exists.

| Built | Not built yet |
|---|---|
| Versioned survey schema (Q1–Q28 + Q19b) | Employee survey UI |
| Five-dimension scoring engine | Personal-result screen |
| Respondent classification ladder | Admin authentication + dashboard |
| Organization aggregation | Pulse creation / duplication / closing |
| 10-rule recommendation engine | Charts and visualisations |
| Opportunity Map analysis | Export download UI |
| Privacy suppression + export shaping | Branding system |
| D1 schema and migrations | Public synthetic demo site |
| 298 unit tests + 7 E2E smoke tests | Full E2E happy paths |

See [docs/phase-0.md](docs/phase-0.md) for the precise scope boundary.
...
```bash
npx wrangler d1 migrations apply pulse_check --local
```
...
- No product UI, so nothing here is usable by a non-technical admin yet.
- Admin authentication is Phase 2; the secret contract exists but no login flow does.
- Only the `/api/health` and `/api/version` endpoints exist. The rest of the API surface arrives with the features that consume it.
- Cloudflare-only. This is a stated V1 limitation, not an oversight.
- Custom questions are validated and modelled but have no authoring UI.
